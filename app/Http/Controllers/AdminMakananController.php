...
class AdminMakananController extends Controller
{
	...
	public function index() {
		...
		return view('admin.list-menu', compact('cabangs'));
	}
	...
	public function fetchAll() {
		...
		->where('kategori','Makanan')->orderBy('is_active','desc')->get();
		$output = '';
		if ($emps->count() > 0) {
	...
	public function fetchAllMakananx(Request $request)
	{
		{
			if($request->ajax())
			{
			 if($request->cabang != '')
			 {
				$data = Menu::select('menu.id','menu.image','menu.nama','menu.deskripsi','menu.amount','cabang.nama_cabang','menu.stock','menu.is_active')
					->join('cabang','cabang.id','menu.cabang_id')
					->where('kategori','Makanan')->orderBy('is_active','desc')
	...
	public function store(Request $request) {
		...
		$empData = [ 'nama' => $request->nama, 'deskripsi' => $request->deskripsi, 'amount' => $request->harga, 'stock' => $request->stock, 'is_active' => $request->is_active, 'image' => $fileName, 'kategori' => 'Makanan', 'cabang_id' => $request->cabang];
		Menu::create($empData);
